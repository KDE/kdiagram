...
void CartesianCoordinatePlane::addDiagram(AbstractDiagram *diagram)
{
    Q_ASSERT_X(dynamic_cast<AbstractCartesianDiagram *>(diagram),
               "CartesianCoordinatePlane::addDiagram",
               "Only cartesian "
               "diagrams can be added to a cartesian coordinate plane!");
    AbstractCoordinatePlane::addDiagram(diagram);
    connect(diagram, &AbstractDiagram::layoutChanged, this, &CartesianCoordinatePlane::slotLayoutChanged);
...
QRectF CartesianCoordinatePlane::getRawDataBoundingRectFromDiagrams() const
{
    ...
    for (const AbstractDiagram *diagram : ds) {
...
QRectF CartesianCoordinatePlane::adjustedToMaxEmptyInnerPercentage(const QRectF &r, unsigned int percentX, unsigned int percentY) const
{
    QRectF ret = r;
...
QRectF CartesianCoordinatePlane::calculateRawDataBoundingRect() const
{
    ...
    const bool bHardHorizontalRange =
        (!bAutoAdjustHorizontalRange) && (d->horizontalMin != d->horizontalMax || (ISNAN(d->horizontalMin) != ISNAN(d->horizontalMax)));
    ...
    dataBoundingRect = adjustedToMaxEmptyInnerPercentage(dataBoundingRect, d->autoAdjustHorizontalRangeToData, d->autoAdjustVerticalRangeToData);
...
DataDimensionsList CartesianCoordinatePlane::getDataDimensionsList() const
{
    ...
    if (dgr) {
        ...
        l.append(DataDimension(r.bottom(),
                               r.top(),
...
void CartesianCoordinatePlane::layoutDiagrams()
{
    ...
    Q_ASSERT_X(d->dimensions.count() == 2, "CartesianCoordinatePlane::layoutDiagrams", "Error: gridDimensionsList() did not return exactly two dimensions.");
...
void CartesianCoordinatePlane::setGridAttributes(Qt::Orientation orientation, const GridAttributes &a)
{
...
{
    return orientation == Qt::Horizontal ? d->hasOwnGridAttributesHorizontal : d->hasOwnGridAttributesVertical;
}
