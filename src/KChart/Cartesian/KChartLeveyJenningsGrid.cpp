...
DataDimensionsList LeveyJenningsGrid::calculateGrid(const DataDimensionsList &rawDataDimensions) const
{
    ...
    if (isBoundariesValid(l)) {
        ...
        if (l.first().isCalculated && plane->autoAdjustGridToZoom() && plane->axesCalcModeX() == CartesianCoordinatePlane::Linear
...
DataDimension LeveyJenningsGrid::calculateGridXY(const DataDimension &rawDataDimension, Qt::Orientation orientation, bool adjustLower, bool adjustUpper) const
{
...
void LeveyJenningsGrid::calculateStepWidth(qreal start_,
                                           qreal end_,
                                           ...
                                           bool adjustUpper) const
{
    ...
    do {
        ...
        calculateSteps(start, end, testList, minSteps, maxSteps, power, steps, stepWidth, adjustLower, adjustUpper);
        --power;
    } while (steps == 0.0);
    ++power;
...
void LeveyJenningsGrid::drawGrid(PaintContext *context)
{
    ...
    if (gridAttrs.isGridVisible(LeveyJenningsGridAttributes::Expected)) {
        ...
        context->painter()->drawLine(plane->translate(QPointF(dimX.start, meanValue)), plane->translate(QPointF(dimX.end, meanValue)));
    ...
    if (gridAttrs.isGridVisible(LeveyJenningsGridAttributes::Calculated)) {
        ...
        context->painter()->drawLine(plane->translate(QPointF(dimX.start, calcMeanValue)), plane->translate(QPointF(dimX.end, calcMeanValue)));
