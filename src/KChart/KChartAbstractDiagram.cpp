...
bool AbstractDiagram::compare(const AbstractDiagram *other) const
{
    ...
    return // compare QAbstractScrollArea properties
        ...
        (rootIndex().column() == other->rootIndex().column()) && (rootIndex().row() == other->rootIndex().row())
        && (allowOverlappingDataValueTexts() == other->allowOverlappingDataValueTexts()) && (antiAliasing() == other->antiAliasing())
        && (percentMode() == other->percentMode()) && (datasetDimension() == other->datasetDimension());
}
...
void AbstractDiagram::setAttributesModel(AttributesModel *amodel)
{
    if (amodel->sourceModel() != model()) {
        qWarning(
            "KChart::AbstractDiagram::setAttributesModel() failed: "
...
{
    const QVariant boolFlag(attributesModel()->data(conditionallyMapFromSource(index), DataHiddenRole));
    if (boolFlag.isValid()) {
        return boolFlag.value<bool>();
    }
...
{
    return attributesModel()->data(conditionallyMapFromSource(index), KChart::DataValueLabelAttributesRole).value<DataValueAttributes>();
}
...
void AbstractDiagram::paintDataValueText(QPainter *painter, const QModelIndex &index, const QPointF &pos, qreal value)
{
...
void AbstractDiagram::paintMarker(QPainter *painter, const DataValueAttributes &a, const QModelIndex &index, const QPointF &pos)
{
    if (!checkInvariants() || !a.isVisible())
        return;
...
void AbstractDiagram::paintMarker(QPainter *painter, const QModelIndex &index, const QPointF &pos)
{
    if (!checkInvariants())
        return;
                                  ...
                                  const QSizeF &maSize)
{
    ...
    if (isFourPixels || (markerAttributes.markerStyle() == MarkerAttributes::Marker1Pixel)) {
        ...
        if (isFourPixels) {
            const qreal x = pos.x();
            const qreal y = pos.y();
            painter->drawLine(QPointF(x - 1.0, y - 1.0), QPointF(x + 1.0, y - 1.0));
            painter->drawLine(QPointF(x - 1.0, y), QPointF(x + 1.0, y));
            painter->drawLine(QPointF(x - 1.0, y + 1.0), QPointF(x + 1.0, y + 1.0));
        }
        ...
        case MarkerAttributes::MarkerSquare: {
            QRectF rect(0 - maSize.width() / 2, 0 - maSize.height() / 2, maSize.width(), maSize.height());
        ...
        case MarkerAttributes::MarkerX: {
            ...
            crossPoints << p1 << p2 << p3 << p4 << p5 << p6 << p7 << p8 << p9 << p10 << p11 << p12;
            painter->drawPolygon(crossPoints);
            break;
        }
        ...
        default:
            Q_ASSERT_X(false, "paintMarkers()", "Type item does not match a defined Marker Type.");
        }
    }
...
{
    return attributesModel()->data(conditionallyMapFromSource(index), DatasetPenRole).value<QPen>();
}
...
{
    return attributesModel()->data(conditionallyMapFromSource(index), DatasetBrushRole).value<QBrush>();
}
...
bool AbstractDiagram::checkInvariants(bool justReturnTheStatus) const
{
    if (!justReturnTheStatus) {
        Q_ASSERT_X(model(), "AbstractDiagram::checkInvariants()", "There is no usable model set, for the diagram.");
