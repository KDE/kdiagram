...
#include "KChartTernaryCoordinatePlane_p.h"
...
void TernaryCoordinatePlane::addDiagram(AbstractDiagram *diagram)
{
    Q_ASSERT_X(dynamic_cast<AbstractTernaryDiagram *>(diagram),
               "TernaryCoordinatePlane::addDiagram",
               "Only ternary "
...
{ // this is our "resize event":
    ...
    QRectF diagramNativeRectangle(QPointF(0.0, 0.0), QSizeF(TriangleWidth, TriangleHeight));
    QPair<QSizeF, QSizeF> margins = grid()->requiredMargins();
    d->diagramRect = areaGeometry();
    diagramNativeRectangle.adjust(-margins.first.width(), -margins.first.height(), margins.second.width(), margins.second.height());
    ...
    // determined by axis label overlap
    {
        ...
        d->diagramRectContainer = d->diagramRect.adjusted(topleft.width(), topleft.height(), -bottomRight.width(), -bottomRight.height());
    }
...
{
    return QPointF(d->diagramRect.bottomLeft().x() + point.x() * d->xUnit, d->diagramRect.bottomLeft().y() + point.y() * d->yUnit);
}
