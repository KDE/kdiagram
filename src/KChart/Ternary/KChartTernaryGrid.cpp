...
{
    return fabs(left.percentage - right.percentage) <= std::numeric_limits<qreal>::epsilon() && left.depth == right.depth;
}
...
void TernaryGrid::drawGrid(PaintContext *context)
{
    ...
    int granularity = 20;
    if (xPixels > 10 * GridLineDistanceThreshold)
        granularity = 10;
    if (xPixels > 20 * GridLineDistanceThreshold)
        granularity = 5;
    ...
    QVector<QLineF> lines[MaxDepth];
    {
        for (const TickInfo &tick : std::as_const(m_tickInfo)) {
            ...
            { // draw parallels to B
                ...
                lines[tick.depth].append(QLineF(plane->translate(start), plane->translate(end)));
            }
            { // draw parallels to C
    ...
    QPolygonF points;
    points << plane->translate(TriangleBottomLeft) << plane->translate(TriangleBottomRight) << plane->translate(TriangleTop);
...

    {
        for (const TickInfo &tick : std::as_const(percentages)) {
            ...
            { // BC axis markers:
                ...
                ticks.append(QLineF(plane->translate(start), plane->translate(start - markerDistance)));
            }
            { // AC axis markers:
                const QPointF markerDistance(FullMarkerDistanceAC / (tick.depth + 1));
                const QPointF start(TriangleBottomRight + percent * AxisVector_C_A);
                const QPointF end(start + markerDistance);
                ticks.append(QLineF(plane->translate(start), plane->translate(end)));
            }
            {
                // AB axis markers:
                const QPointF markerDistance(FullMarkerDistanceBA / (tick.depth + 1));
