...
{
    if (a >= 0.0 && a <= 1.0 && b >= 0.0 && b <= 1.0 && 1.0 - a - b >= -2.0 * std::numeric_limits<qreal>::epsilon()) {
        m_a = a;
        m_b = b;
...
{
    return m_a >= 0.0 && m_a <= 1.0 && m_b >= 0.0 && m_b <= 1.0 && 1.0 - m_a + m_b >= -std::numeric_limits<qreal>::epsilon();
}
...
QDebug operator<<(QDebug stream, const TernaryPoint &point)
{
    ...
    if (point.isValid()) {
        ...
        text << (int)(point.a() * 100.0) << "%|" << (int)(point.b() * 100.0) << "%|" << (int)(point.c() * 100.0) << "%]";
    } else {
