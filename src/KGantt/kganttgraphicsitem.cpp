...
GraphicsItem::GraphicsItem(const QModelIndex &idx, QGraphicsItem *parent, GraphicsScene *scene)
    : BASE(parent)
    , m_index(idx)
    , m_isupdating(false)
{
...
void GraphicsItem::updateModel()
{
    ...
    if (isEditable()) {
        ...
        if (model) {
            ...
            if (scene()->getGrid()->mapFromChart(Span(scenePos().x(), rect().width()), index(), constraints)) {
...
void GraphicsItem::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (scene()->dragSource()) {
        ...
        if (other && scene()->dragSource() != other && other->index().data(KGantt::ItemTypeRole) == KGantt::TypeEvent) {
            ...
            if (other->mapToScene(itemRect).boundingRect().contains(event->scenePos())) {
                ...
                if (view) {
                    view->addConstraint(scene()->summaryHandlingModel()->mapToSource(scene()->dragSource()->index()),
                                        scene()->summaryHandlingModel()->mapToSource(other->index()),
                                        event->modifiers());
                }
            }
        } else {
...
void GraphicsItem::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    case ItemDelegate::State_Move:
        ...
        if (qAbs(m_pressscenepos.x() - event->scenePos().x()) < 10. && qAbs(m_pressscenepos.y() - event->scenePos().y()) > 5.) {
